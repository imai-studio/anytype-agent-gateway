...
import { BridgeDeliverySchema, BridgeInboundSchema, } from "./protocol.js";
export class BridgeStore {
    ...
    pendingInbound(limit = 50) {
        const rows = this.#db
            .prepare(`SELECT payload FROM bridge_inbound WHERE status = 'pending'
         ORDER BY created_at ASC LIMIT ?`)
            .all(limit);
        return rows.map((row) => BridgeInboundSchema.parse(JSON.parse(String(row.payload))));
    }
    markInbound(id, status, error, now = Date.now()) {
        this.#db
    ...
    bindSession(sessionKey, accountId, route, now = Date.now()) {
        this.#db
            .prepare(`INSERT INTO bridge_bindings(session_key, account_id, route, updated_at)
         VALUES (?, ?, ?, ?)
    ...
    bindingForSession(sessionKey) {
        ...
        return {
            ...
            route: JSON.parse(String(row.route)),
        };
    }
    ...
    pendingDeliveries(now = Date.now(), limit = 100, afterSequence = 0) {
        const rows = this.#db
            .prepare(`SELECT rowid AS store_sequence, payload, attempts, next_attempt_at, last_error FROM bridge_outbound
         WHERE status = 'pending' AND next_attempt_at <= ?
           AND rowid > ?
         ORDER BY rowid ASC LIMIT ?`)
            .all(now, afterSequence, limit);
        return rows.map((row) => ({
    ...
    pendingDeliveriesFor(filter, now = Date.now(), limit = 100, afterSequence = 0) {
        const rows = this.#db
            .prepare(`SELECT rowid AS store_sequence, payload, attempts, next_attempt_at, last_error FROM bridge_outbound
         WHERE status = 'pending' AND next_attempt_at <= ?
           AND rowid > ?
         ...
         ORDER BY rowid ASC LIMIT ?`)
            .all(now, afterSequence, filter.sessionKey, filter.route.spaceId, filter.route.chatId, filter.route.discussionRootId ?? "", limit);
        return rows.map((row) => ({
            ...BridgeDeliverySchema.parse(JSON.parse(String(row.payload))),
            storeSequence: Number(row.store_sequence),
            attempts: Number(row.attempts),
    ...
    acknowledgeDeliveries(ids, now = Date.now()) {
        if (ids.length === 0)
            return;
    ...
    retryDelivery(id, error, now = Date.now()) {
        ...
        this.#db
            .prepare(`UPDATE bridge_outbound SET attempts = ?, next_attempt_at = ?, last_error = ?, updated_at = ?
         WHERE id = ?`)
