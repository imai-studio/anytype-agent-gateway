...
`@imai/openclaw-anytype-channel` makes an Anytype chat or object discussion a native OpenClaw channel. Knot remains the Anytype transport owner; this plugin owns OpenClaw session routing and observes native agent output.
...
- The sender allowlist is checked again inside the OpenClaw process, even if Knot already checked it.
...
openclaw plugins install /absolute/path/to/knot/packages/openclaw-anytype-channel
```
...
The bridge is pull-only: Knot reads its durable loopback outbox, then acknowledges records after they are durable in Knot.

Restart the OpenClaw gateway after enabling the plugin. The standard Knot runtime keeps using the OpenClaw Gateway for starts and steering, then registers that exact operator session through `POST /v1/bindings`. The bridge also supports native-channel ingress at `POST /v1/inbound` for hosts that use the package directly.
...
This hybrid path is what the bundled Knot runtime uses. It preserves native `sessions.steer` and makes events from later OpenClaw cron, heartbeat, subagent, and session-continuation runs observable on the same route without starting a second OpenClaw session.

After OpenClaw returns a run ID, Knot registers it through authenticated `POST /v1/owned-runs`. The plugin persists that ownership marker and attaches it to the run's deliveries, so a Knot restart between projection and acknowledgement cannot mirror the operator run as a second proactive reply.
...
The response is `202` for a new event or `200` for a duplicate. A failed dispatch can be retried by posting the same ID again. Check `GET /v1/inbound/:id` when Knot needs a delivery receipt.
...
The payload kind is either `agent-event` or `message-final`. `agent-event` preserves native `runId`, `seq`, stream, timestamp, and sanitized event data. `message-final` is the channel's durable final delivery. Knot should project agent events for live thinking/text and reconcile the final delivery into the same output cycle rather than creating a duplicate message.
...
- Text, replies, and discussions are implemented. Media upload and Anytype reactions remain Knot responsibilities.
- Knot must implement the delivery endpoint or poll the outbox. The standard Knot runtime polls and acknowledges it. The plugin deliberately does not receive an Anytype API key.
- OpenClaw emits sanitized thinking events, not hidden model chain-of-thought. Knot may display only the safe progress text present in those events.
- The final channel delivery and assistant event stream describe the same run. Knot must deduplicate using the idempotency key and reconcile by session/run/output cycle.
