...
type ChatCreator = {
  createChat(spaceId: string, input: { name: string }): Promise<{ id: string; name: string }>;
};

type TaskCreator = typeof createCodexTask;
type RouteEnroller = typeof enrollChatRoute;

export async function createBoundCodexChat(
  anytype: ChatCreator,
  config: AgentConfig,
  configPath: string,
  input: { spaceId: string; name: string; project: string; prompt: string },
  dependencies: { createTask?: TaskCreator; enrollRoute?: RouteEnroller } = {},
): Promise<{
  chat_id: string;
  chat_name: string;
  route_id: string;
  ...
  let task: Awaited<ReturnType<TaskCreator>>;
  try {
    ...
    throw new Error(
      `Anytype chat ${chat.id} was created, but its Codex task could not be created: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    const enrollment = await (dependencies.enrollRoute ?? enrollChatRoute)({
      ...
      chatId: chat.id,
      chatName: chat.name,
      wake: configuredSpace.chatDiscovery.wake as WakeConfig,
    });
    if (enrollment === "disabled") throw new Error("chat route enrollment is disabled");
  } catch (error) {
    storeBindingCleanup(config.state.path, routeId, sessionKey);
    throw new Error(
      `Anytype chat ${chat.id} and Codex task ${task.thread_id} were created, but the route could not be enrolled: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
function boundTaskPrompt(config: AgentConfig, sessionKey: string, prompt: string): string {
...
function storeBindingCleanup(statePath: string, routeId: string, sessionKey: string): void {
