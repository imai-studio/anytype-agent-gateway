...
const anytypePropertySchema = z
  ...
  .superRefine((value, context) => {
    if (
      ["archived", "id", "layout", "object", "space_id", "type", "type_key"].includes(
    ...
    const fields = [
      ...
      "phone",
      "objects",
    ].filter((field) => value[field as keyof typeof value] !== undefined);
    if (fields.length !== 1)
      context.addIssue({
        code: "custom",
...
function requiredWorkflowStep<
  K extends z.infer<typeof workflowStepKindSchema>,
...
const workflowDefinitionObjectSchema = z
  ...
  .superRefine((workflow, context) => {
    ...
    for (const [index, step] of workflow.spec.steps.entries()) {
      ...
      if (!config || !("inputStepId" in config) || !config.inputStepId) continue;
...
export function isSensitiveWorkflowTextPath(path: readonly string[]): boolean {
  const key = path.at(-1);
  if (key === "prompt" || key === "message") return true;
  if (
    path.length === 6 &&
    path[0] === "spec" &&
    path[1] === "steps" &&
