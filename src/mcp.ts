...
type Request = { jsonrpc?: string; id?: string | number | null; method?: string; params?: Record<string, any> };
type Tool = { name: string; description: string; inputSchema: Record<string, unknown> };

export async function runMcpServer(configPath: string, context: { routeId?: string; spaceId?: string } = {}): Promise<void> {
  const config = await loadConfig(configPath);
  if (!config.tools.anytype.enabled) throw new Error("Anytype tools are disabled in this AAG configuration");
  ...
  for await (const line of lines) {
    ...
    try {
      let result: unknown;
      if (request.method === "initialize") result = { protocolVersion: request.params?.protocolVersion ?? "2025-06-18", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "aag-anytype", version: VERSION } };
...
export async function callTool(anytype: AnytypeClient, config: AgentConfig, configPath: string, routeId: string | undefined, defaultSpaceId: string | undefined, name: string, input: Record<string, any>): Promise<unknown> {
  if (name === "aag_context") return {
  ...
  if (name === "aag_set_wake") {
    ...
    if (!routeId) throw new Error("This tool was not launched with an Anytype route context");
  ...
  if (name === "anytype_get_object") return withLink(await anytype.getObject(spaceId, required(input, "object_id")), spaceId);
  if (!config.tools.anytype.allowWrite) throw new Error("Anytype writes are disabled for this agent");
  if (name === "anytype_create_object") return withLink(await anytype.createObject(spaceId, pick(input, ["type_key", "name", "body", "template_id", "properties"], ["type_key"])), spaceId);
  if (name === "anytype_update_object") return withLink(await anytype.updateObject(spaceId, required(input, "object_id"), pick(input, ["type_key", "name", "markdown", "properties"])), spaceId);
  if (name === "anytype_add_to_list") { await anytype.addObjectsToList(spaceId, required(input, "list_id"), requiredArray(input, "object_ids")); return { added: input.object_ids.length, list: anytypeLink(spaceId, input.list_id) }; }
  if (name === "anytype_upload_file") { const path = await allowedFile(config, required(input, "path")); const file = await anytype.uploadFile(spaceId, path); return { ...file, link: anytypeLink(spaceId, String(file.object_id ?? "")) }; }
...
function assertSpaceAllowed(config: AgentConfig, spaceId: string, defaultSpaceId: string | undefined): void {
  const explicit = config.tools.anytype.allowedSpaceIds;
  if (explicit.length) {
    if (!explicit.includes(spaceId)) throw new Error(`Space ${spaceId} is not allowed for this agent`);
    return;
  }
...
async function allowedFile(config: AgentConfig, value: string): Promise<string> {
  ...
  const configured = config.tools.anytype.allowedFileRoots.length ? config.tools.anytype.allowedFileRoots : [config.runtime.defaultProject, ...config.runtime.allowedProjects].filter(Boolean) as string[];
...
function withLink<T extends Record<string, any>>(object: T, spaceId: string): T & { link: string } { return { ...object, link: anytypeLink(spaceId, String(object.id ?? object.object_id ?? "")) }; }
function anytypeLink(spaceId: string, objectId: string): string { return `anytype://object/?objectId=${encodeURIComponent(objectId)}&spaceId=${encodeURIComponent(spaceId)}`; }
function required(input: Record<string, any>, key: string): string { const value = input[key]; if (typeof value !== "string" || !value) throw new Error(`${key} is required`); return value; }
function requiredArray(input: Record<string, any>, key: string): string[] { if (!Array.isArray(input[key]) || input[key].length === 0) throw new Error(`${key} is required`); return input[key].map(String); }
function pick(input: Record<string, any>, keys: string[], requiredKeys: string[] = []): any { const value: Record<string, any> = {}; for (const key of keys) if (input[key] !== undefined) value[key] = input[key]; for (const key of requiredKeys) if (value[key] === undefined) throw new Error(`${key} is required`); return value; }
function objectSchema(properties: Record<string, unknown>, required: string[] = []): Record<string, unknown> { return { type: "object", properties, additionalProperties: false, ...(required.length ? { required } : {}) }; }
function stringSchema(description?: string): Record<string, unknown> { return { type: "string", ...(description ? { description } : {}) }; }
function rpcError(code: number, message: string): Error & { code: number } { return Object.assign(new Error(message), { code }); }
function write(value: unknown): void { process.stdout.write(`${JSON.stringify(value)}\n`); }
function spaceFromRoute(routeId?: string): string | undefined { return /^(?:aag:)?(?:chat|discussion):([^:]+)/.exec(routeId ?? "")?.[1]; }
function baseRoute(routeId: string): string { return routeId.replace(/^aag:/, "").replace(/:g\d+$/, "").replace(/:root:.+$/, ""); }
