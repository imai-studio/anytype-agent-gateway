...
export function toolDefinitions(config: AgentConfig): Tool[] {
  ...
  return [
    ...
    },
    {
      ...
      description:
        "Update an existing Anytype object's name, Markdown body, or properties without changing its type. Read it first and preserve unrelated values. Returns a native link and Knot object reference.",
      inputSchema: objectSchema(
        {
          space_id: stringSchema(),
    ...
    },
    {
      ...
      description:
        "Set this agent identity's own Anytype profile image from an allowed local image file. The target identity is fixed by Knot and cannot be changed by the caller.",
      inputSchema: objectSchema(
...
export async function callTool(
  ...
  if (name === "aag_context")
    return {
      ...
      object_links:
        "Use object_ref for a compact clickable mention. Use object_card when the user asks to send, attach, or show the object itself; Knot renders it as a native Anytype card. link is the deep-link fallback",
      object_workflow:
  ...
  if (name === "aag_set_profile_image") {
    ...
    if (!memberId)
      throw new Error(
        "This space needs spaces[].participantId or agent.participantId before Knot can update its own profile",
      );
...
function schedulingContext(
  ...
  if (config.runtime.kind !== "openclaw")
    return {
      ...
      available: false,
      reason:
        "This Codex ACP connection has no native scheduled-task integration; Knot does not emulate a scheduler",
    };
  if (!routeId)
    return {
      provider: "openclaw",
