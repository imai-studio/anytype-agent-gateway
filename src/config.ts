...
const disabledWake = { humans: "disabled" as const, agents: "never" as const, allowedUsers: ["__disabled__"] };
...
const commentsSchema = z.object({
  ...
  if (value.mode !== "disabled" && !value.wake) context.addIssue({ code: "custom", path: ["wake"], message: "comments.wake is required when comments are enabled" });
  if (value.mode === "filtered" && value.includeObjectTypes.length === 0) context.addIssue({ code: "custom", path: ["includeObjectTypes"], message: "comments.includeObjectTypes is required for filtered mode" });
...
export async function loadConfig(path: string): Promise<AgentConfig> {
  ...
  if (config.anytype.cli.configPath) config.anytype.cli.configPath = expandHome(config.anytype.cli.configPath);
