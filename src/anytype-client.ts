...
export class AnytypeClient implements AnytypePort {
  ...
  private constructor(private readonly base: string, private readonly headers: Record<string, string>, private readonly participantId?: string) {}
  ...
  private async requestWithRetry(path: string, init: RequestInit, method: string, streaming: boolean): Promise<Response> {
  ...
  async resolveSpace(selector: { id?: string; name?: string }): Promise<{ id: string; name: string }> {
    const spaces = await this.listPages("/v1/spaces");
    const matches = spaces.filter((space: JsonRecord) => selector.id ? space.id === selector.id : space.name === selector.name);
    if (matches.length !== 1) throw new Error(`Expected one Anytype space matching ${selector.id ?? JSON.stringify(selector.name)}, found ${matches.length}`);
  ...
  async getMessage(spaceId: string, chatId: string, messageId: string): Promise<ChatMessage> {
    const json = await (await this.request(this.messagePath(spaceId, chatId, messageId))).json() as { message: ChatMessage };
    return json.message;
  }
  ...
  async listMessages(spaceId: string, chatId: string, limit: number, afterOrderId?: string): Promise<ChatMessage[]> {
    ...
    const json = await (await this.request(`${this.messagesPath(spaceId, chatId)}?${query}`)).json() as { messages?: ChatMessage[] };
    return json.messages ?? [];
  }

  async sendMessage(spaceId: string, chatId: string, input: { text: string; replyTo?: string; marks?: TextMark[] }): Promise<string> {
    ...
    const json = await (await this.request(this.messagesPath(spaceId, chatId), { method: "POST", body: JSON.stringify(body) })).json() as { message_id?: string };
    if (!json.message_id) throw new Error("Anytype returned no message_id");
    return json.message_id;
  }

  async editMessage(spaceId: string, chatId: string, messageId: string, text: string, marks?: TextMark[]): Promise<void> {
  ...
  async ensureReaction(spaceId: string, chatId: string, messageId: string, emoji: string, present: boolean): Promise<void> {
    ...
    if (present) this.localReactions.add(key); else this.localReactions.delete(key);
  }
  ...
  async getObject(spaceId: string, objectId: string): Promise<{ id: string; name?: string; markdown?: string }> {
    const json = await (await this.request(`/v1/spaces/${encodeURIComponent(spaceId)}/objects/${encodeURIComponent(objectId)}`)).json() as JsonRecord;
  ...
  private messagePath(spaceId: string, chatId: string, messageId: string): string { return `${this.messagesPath(spaceId, chatId)}/${encodeURIComponent(messageId)}`; }
  ...
  private async listPages(path: string): Promise<JsonRecord[]> {
    ...
    for (let offset = 0, pageNumber = 0; pageNumber < 100; pageNumber += 1) {
      ...
      const json = await (await this.request(`${path}${separator}offset=${offset}&limit=100`)).json() as JsonRecord;
      if (!Array.isArray(json.data)) throw new Error(`Anytype ${path} returned an invalid list payload`);
      const page = (json.data as JsonRecord[]).filter(item => typeof item?.id === "string" && !seen.has(item.id));
      for (const item of page) { seen.add(item.id); items.push(item); }
...
async function readWithIdleTimeout(reader: ReadableStreamDefaultReader<Uint8Array>, signal: AbortSignal, timeoutMs: number): Promise<ReadableStreamReadResult<Uint8Array>> {
  let timer: NodeJS.Timeout | undefined;
  let abort: (() => void) | undefined;
  const timeout = new Promise<never>((_resolve, reject) => { timer = setTimeout(() => reject(new Error(`Anytype event stream idle for ${Math.round(timeoutMs / 1000)} seconds`)), timeoutMs); });
...
export function parseSseBlock(block: string): AnytypeEvent | undefined {
  ...
  if (!value || typeof value !== "object" || typeof (value as { type?: unknown }).type !== "string") throw new Error("Anytype event stream returned an invalid event envelope");
