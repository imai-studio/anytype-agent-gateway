...
const directMessageSpaceMarker = (spaceId: string) => `system:aag:direct-message-space:${spaceId}`;
const directMessageBootstrapMarker = (identity: string) =>
  `system:aag:direct-message-bootstrap:${identity}`;
...
export class Gateway {
  ...
  private readonly reportedUnknownSpaceKinds = new Set<string>();
  private initialDirectMessageScanComplete = false;
  private readonly directMessageBootstrapFailures = new Map<
    string,
    { failures: number; nextAttemptAt: number }
  >();
  private readonly directMessageMembership = new Map<
    string,
    {
      checkedAt: number;
      authorized: boolean;
  ...
  private async processMessage(route: Route, message: ChatMessage): Promise<void> {
    const effectiveWake = mergeWakeOverride(
      route.wake,
      this.store.wakeOverride(route.conversation.routeId),
    );
  ...
  private async maybeAutoEnroll(
    route: Route,
    wake: WakeConfig,
    message: ChatMessage,
  ): Promise<void> {
    const enrollment = route.autoEnrollment;
    if (!enrollment || enrollment.complete || !this.enrollChat) return;
    if (enrollment.pending) return enrollment.pending;
    if (Date.now() < enrollment.nextAttemptAt) return;
    ...
    enrollment.pending = (async () => {
      try {
        const result = await this.enrollChat!(
          route.conversation.spaceId,
          route.conversation.spaceName ?? "",
  ...
  private async discoverDirectMessages(discovery: AgentConfig["directMessages"]): Promise<void> {
    while (!this.abort.signal.aborted) {
      try {
        ...
        const newlyObservedKinds = unknownKinds.filter(
          (kind) => !this.reportedUnknownSpaceKinds.has(kind),
        );
        if (newlyObservedKinds.length) {
          for (const kind of newlyObservedKinds) this.reportedUnknownSpaceKinds.add(kind);
        ...
        if (!this.initialDirectMessageScanComplete) {
          if (!this.store.isInitialized(DIRECT_MESSAGE_DISCOVERY_MARKER)) {
            for (const space of spaces) this.store.initialize(directMessageSpaceMarker(space.id));
            this.store.initialize(DIRECT_MESSAGE_DISCOVERY_MARKER);
          }
          this.initialDirectMessageScanComplete = true;
        }
        let chatsDiscovered = 0;
        const authorizedPeerIdentities = new Set<string>();
        ...
        for (const space of spaces) {
          try {
            ...
            const refreshAfter =
              membership?.authorized || membership?.reason === "unauthorized-peer"
                ? 5 * 60 * 1000
                : discovery.discoveryIntervalSeconds * 1000;
            if (!membership || now - membership.checkedAt >= refreshAfter) {
              const members = (await this.anytype.listMembers(space.id)).filter(
                (member) => member.status === "active",
              );
              const self = members.find((member) =>
                sameIdentity(member.identity ?? member.id, this.config.agent.participantId),
              );
              ...
              const authorizedPeer =
                peers.length === 1 &&
                discovery.wake.allowedUsers.some((allowed) =>
            ...
            if (!membership.authorized || !membership.selfParticipantId) continue;
            if (membership.peerIdentity) authorizedPeerIdentities.add(membership.peerIdentity);
        ...
        if (discovery.createMissing && membershipScanClean) {
          for (const allowed of discovery.wake.allowedUsers) {
            ...
            if (
              ...
              continue;
            try {
              const created = await this.discussions.ensureDirectMessage(
                identity,
                this.abort.signal,
              );
              const members = (await this.anytype.listMembers(created.spaceId)).filter(
                (member) => member.status === "active",
              );
              const self = members.find((member) =>
                sameIdentity(member.identity ?? member.id, this.config.agent.participantId),
              ...
              const peer = members.find((member) =>
                sameIdentity(member.identity ?? member.id, identity),
              );
