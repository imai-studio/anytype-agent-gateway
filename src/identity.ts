...
export async function createIdentity(options: { command: string; name: string; invites: string[]; apiKeyFile: string; dataPath?: string }): Promise<void> {
  ...
  for (const invite of options.invites) await inherited(options.command, ["space", "join", invite], env);
  ...
  if (!match?.[1]) throw new Error("Anytype CLI created an API key, but AAG could not recognize its output");
...
export async function joinSpaces(command: string, invites: string[], dataPath?: string): Promise<void> {
