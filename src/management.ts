...
export async function setRouteWake(input: { configPath: string; routeId: string; humans: string; prefix?: string }): Promise<void> {
  if (!humanModes.includes(input.humans as HumanMode)) throw new Error(`Invalid human wake mode: ${input.humans}`);
  if (input.humans === "prefix" && !input.prefix) throw new Error("--prefix is required when --humans is prefix");
  const match = /^(chat|discussion):([^:]+):(.+)$/.exec(input.routeId);
  if (!match) throw new Error("Route ID must be chat:<space-id>:<chat-id> or discussion:<space-id>:<discussion-id>");
  const kind = match[1] as "chat" | "discussion";
  const spaceId = match[2]!;
  const id = match[3]!;
  const absolute = resolve(input.configPath);
  const extension = extname(absolute).toLowerCase();
  if (extension === ".toml") throw new Error("Wake self-management currently supports YAML and JSON configuration files");
  const raw = parseConfig(await readFile(absolute, "utf8"), extension) as Record<string, unknown>;
  const parsed = configSchema.parse(raw);
  if (!parsed.management.allowWakeChanges) throw new Error("management.allowWakeChanges is disabled");
  ...
  const overrides = Array.isArray(rawSpace.wakeOverrides) ? rawSpace.wakeOverrides as Array<Record<string, unknown>> : [];
  ...
  try { store.setWakeOverride(input.routeId, input.humans); }
  finally { store.close(); }
}

function parseConfig(text: string, extension: string): unknown { return extension === ".json" ? JSON.parse(text) : YAML.parse(text); }
function serializeConfig(value: unknown, extension: string): string { return extension === ".json" ? `${JSON.stringify(value, null, 2)}\n` : YAML.stringify(value); }
