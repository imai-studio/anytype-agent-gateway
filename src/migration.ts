...
export function migrationPaths(
  ...
  if (!statePath.includes(stateMarker))
    throw new Error(
      `Legacy state path is outside the migratable AAG state tree: ${statePath}. The selected --config must set state.path beneath ~/.local/state/aag, including its per-agent directory when applicable.`,
    );
  const stateSource = dirname(statePath);
  const stateDestination = dirname(
    statePath.replace(stateMarker, `${sep}.local${sep}state${sep}knot${sep}`),
  );
  const { configSource, configDestination } = legacyConfigLayout(
    home,
    legacyConfigPath ?? join(home, ".config", "aag", "agent.yaml"),
  );
  const paths: Array<[MigrationItem["kind"], string, string]> = [
...
export async function migrateInstallation(
  ...
  for (const item of items) {
    if (!(await exists(item.source))) item.status = "missing";
    else if (await exists(item.destination)) {
      await verifyTree(
        item.source,
        item.destination,
        item.kind === "config" ? configTransform : undefined,
      );
  ...
  if (options.dryRun) return { version: 1, dryRun: true, state: "planned", items, rollback };
...
export async function latestMigrationManifest(
  home = homedir(),
  ...
  for (const name of names.reverse()) {
    ...
    let value: MigrationResult;
    try {
      value = JSON.parse(await readFile(path, "utf8")) as MigrationResult;
    } catch {
      if (expectedConfigSource) continue;
      throw new Error(`Invalid migration manifest: ${path}`);
    }
    const configSource = value.items?.find((item) => item.kind === "config")?.source;
    if (expectedConfigSource && configSource !== expectedConfigSource) continue;
...
async function configTransform(
  source: string,
  destination: string,
  contents: Buffer,
): Promise<Buffer> {
  if (basename(source) !== "agent.yaml") return contents;
  const document = rewriteLegacyPaths(parse(contents.toString("utf8"))) as Record<string, unknown>;
  const state =
  ...
  if (typeof state.path !== "string") {
    ...
    if (index < 0) throw new Error(`Could not derive the migrated state path from ${source}`);
...
function rewriteLegacyPaths(value: unknown): unknown {
  ...
  if (Array.isArray(value)) return value.map(rewriteLegacyPaths);
  if (value && typeof value === "object")
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, rewriteLegacyPaths(item)]),
    );
  return value;
}

type Transform = (source: string, destination: string, contents: Buffer) => Promise<Buffer>;
...
async function copyTreeAtomic(
  ...
  if (!sourceInfo.isDirectory()) throw new Error(`Migration source is not a directory: ${source}`);
...
async function copyDirectoryContents(
  ...
  for (const entry of await readdir(source, { withFileTypes: true })) {
    if (ignoredRuntimeFile(entry.name)) continue;
    const from = join(source, entry.name);
    const to = join(destination, entry.name);
    const info = await lstat(from);
    if (info.isSymbolicLink()) throw new Error(`Refusing symbolic link: ${from}`);
    if (info.isDirectory()) {
      ...
      if (contents) await writeAtomic(to, contents, info.mode & 0o7777);
      else {
    ...
    } else throw new Error(`Refusing non-regular migration source: ${from}`);
  }
}
...
async function verifyTree(
  ...
  for (const name of sourceFiles) {
    ...
    if (name.endsWith(".sqlite")) await verifySqlite(to);
  }
}

async function verifySqlite(destination: string): Promise<void> {
...
async function resolvedLegacyStatePath(home: string, configPath: string): Promise<string> {
  let explicit: string | undefined;
  if (await exists(configPath)) {
    const raw = parse(await readFile(configPath, "utf8")) as { state?: { path?: unknown } };
    if (typeof raw.state?.path === "string") explicit = raw.state.path;
  }
  return resolveStatePath({ ...(explicit ? { explicit } : {}), home });
}

function legacyConfigLayout(
  home: string,
  configPath: string,
): {
...
async function resolveLegacyConfigSelection(home: string, requested?: string): Promise<string> {
  ...
  if (!requested && !(await exists(layout.selectedConfig))) {
    ...
    if (candidates.length > 0)
      throw new Error(
        `No default legacy configuration exists at ${defaultConfig}; found ${candidates.length} nested agent.yaml ${candidates.length === 1 ? `at ${candidates[0]}` : "files"}. Rerun with --config <legacy agent.yaml>.`,
      );
...
export async function resolveLegacyConfigSource(
  home = homedir(),
  requested?: string,
): Promise<string> {
...
async function nestedLegacyConfigs(root: string): Promise<string[]> {
  ...
  async function visit(directory: string): Promise<void> {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      else if (info.isFile() && entry.name === "agent.yaml") output.push(path);
    }
  }
  await visit(root);
  return output.sort();
...
function sqliteSnapshot(path: string): unknown {
  const db = new DatabaseSync(path);
  try {
    const quick = db.prepare("PRAGMA quick_check").all();
    const integrity = db.prepare("PRAGMA integrity_check").all();
    if (
      JSON.stringify(quick) !== '[{"quick_check":"ok"}]' ||
...
async function regularFiles(root: string): Promise<string[]> {
  ...
  async function visit(directory: string): Promise<void> {
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      const path = join(directory, entry.name);
      const info = await lstat(path);
      if (info.isSymbolicLink()) throw new Error(`Refusing symbolic link: ${path}`);
      if (info.isDirectory()) await visit(path);
      else if (info.isFile()) output.push(relative(root, path));
      else throw new Error(`Refusing non-regular path: ${path}`);
    }
  }
  await visit(root);
  return output.sort();
...
function ignoredRuntimeFile(name: string): boolean {
...
async function ensureSafeDestination(destination: string): Promise<void> {
...
async function assertSafeHome(home: string): Promise<void> {
...
async function writeAtomic(path: string, contents: string | Buffer, mode: number): Promise<void> {
