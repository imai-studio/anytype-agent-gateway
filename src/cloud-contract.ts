...
const publicationSlugSchema = z
  ...
  .refine(
    (value) => !["api", "_next", "www", "admin", "health", "assets"].includes(value.split("/")[0]!),
    ...
    requiredHeaders: z
      .record(
        z
          ...
          .refine(
            (name) => !["authorization", "cookie", "host", "proxy-authorization"].includes(name),
...
function encodeCanonicalJson(value: JsonValue, depth: number): string {
  if (depth > 100) throw new TypeError("Canonical JSON exceeds the maximum depth");
