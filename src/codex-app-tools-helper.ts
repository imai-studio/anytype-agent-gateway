...
async function main(): Promise<void> {
  ...
  try {
    const catalog = (await client.request("tools/list", { threadStartKind: "all" })) as {
      tools?: ToolDescriptor[];
    };
    if (input.action === "hydrate") {
      if (!input.threadId) throw new Error("Codex Desktop hydration requires a task ID");
    ...
    if (!createTool?.namespace) throw new Error("Codex Desktop does not expose create_thread");
...
async function callTool(
  client: CodexAppHostClient,
  tool: Required<Pick<ToolDescriptor, "name" | "namespace">>,
  sourceThreadId: string,
...
class CodexAppHostClient {
  ...
  constructor(pipePath: string, timeoutMs: number) {
