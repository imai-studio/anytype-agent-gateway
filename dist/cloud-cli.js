...
import { cloudFileMode, forgetCloudIdentity, initializeCloudConfig, loadCloudConfig, loadPairingCredentials, normalizeCloudBaseUrl, removePairingCredentials, resolveCloudPaths, saveCloudConfig, savePairingCredentials, validateCloudKey, } from "./cloud-config.js";
...
export const DEFAULT_CLOUD_SCOPES = ["anytype.objects.read"];
export async function cloudLogin(input, context = {}) {
...
export async function cloudPair(input, context = {}) {
    const output = context.output ?? console.log;
    ...
    if (input.credentialsFile) {
        const raw = input.credentialsFile === "-"
        ...
        try {
            ...
            output("Then pass the copied one-time JSON with `--credentials-file <file>` or stdin with `--credentials-file -`.");
            throw error;
        }
    }
    ...
    while (true) {
        ...
        if (status.status === "approved") {
            const unexpectedScopes = status.grant.scopes.filter((scope) => !config.requestedScopes.includes(scope));
            const unexpectedSlugs = status.grant.slugGrants.filter((slug) => !config.requestedSlugGrants.includes(slug));
            if (unexpectedScopes.length || unexpectedSlugs.length)
                throw new Error("Knot Cloud approved access that this machine did not request. Revoke the connector in the dashboard and start a new pairing");
        ...
        if (status.status === "consumed") {
            throw new Error("The one-time pairing result was already consumed before it could be saved locally. Revoke any connector created by this attempt in the dashboard, then create a new pairing request");
        }
...
export async function cloudStatus(input, context = {}) {
    const output = context.output ?? console.log;
    const paths = resolveCloudPaths({
        ...(input.configFile ? { configFile: input.configFile } : {}),
        environment: process.env,
    ...
    if (input.json)
        ...
        output(config.paired ? `Pairing: active (${config.paired.connectorId})` : "Pairing: not configured");
        if (config.paired)
            output(`Granted scopes: ${config.paired.scopes.join(", ")}`);
        if (serverError)
            output(`Server check: ${serverError}`);
    }
    return status;
}
export async function cloudDoctor(input, context = {}) {
    const output = context.output ?? console.log;
    const paths = resolveCloudPaths({
        ...(input.configFile ? { configFile: input.configFile } : {}),
        environment: process.env,
...
export async function cloudRevoke(input, context = {}) {
    ...
    if (!input.forgetLocal) {
        output(`Revoke connector ${config.paired?.connectorId ?? "for this machine"} at ${dashboardUrl(config.baseUrl)}.`);
        output("Remote revocation requires an authenticated workspace owner or admin.");
        output("After remote revocation, run `knot cloud revoke --forget-local` to remove this machine's key and cloud configuration.");
        return;
    }
    await forgetCloudIdentity(paths);
    output("Removed the local Cloud configuration, pairing state, and connector private key.");
    output("This does not revoke a remote connector. Confirm remote revocation in the Cloud dashboard.");
}
...
function assertPairingOrigin(baseUrl, credentials) {
...
function assertProtocolCompatible(minimum, maximum) {
    if (compareProtocolVersions(CLOUD_PROTOCOL_VERSION, minimum) < 0 ||
        compareProtocolVersions(CLOUD_PROTOCOL_VERSION, maximum) > 0)
        throw new Error(`Knot Cloud supports protocol ${minimum} through ${maximum}; this CLI supports ${CLOUD_PROTOCOL_VERSION}`);
}
function compareProtocolVersions(left, right) {
    ...
    if (!leftMatch || !rightMatch)
        throw new Error("Knot Cloud returned an invalid protocol range");
...
async function requiredCloudConfig(paths) {
    const config = await loadCloudConfig(paths);
    if (!config)
        throw new Error("Run `knot cloud login` first");
    return config;
}
