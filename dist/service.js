...
import { access, chmod, mkdir, open, readFile, readdir, rename, unlink, writeFile, } from "node:fs/promises";
...
export async function migrateService(options = {}) {
    ...
    try {
        ...
        if (oldFinal.enabled ||
            ...
            !newFinal.enabled ||
            !newFinal.running)
            throw new Error("Service migration health check did not prove exactly one enabled, running Knot service");
        ...
        if (legacyStopped) {
            ...
            if (rollbackErrors.length)
                throw new AggregateError([error, ...rollbackErrors], `Service migration failed and automatic rollback was incomplete. Legacy backup: ${backup ?? "definition was not yet backed up"}. Follow the manual rollback guide.`);
        }
        throw error;
    }
}
...
function platformServiceMigrationManager(home) {
    if (process.platform === "linux")
        return systemdMigrationManager(home);
    if (process.platform === "darwin")
        return launchdMigrationManager(home);
    throw new Error("Service migration supports Linux systemd and macOS launchd only");
}
function systemdMigrationManager(home) {
    ...
    return {
        async inspect(generation) {
            const identity = generation === "aag" ? PRODUCT.services.linux.legacy : PRODUCT.services.linux.current;
        ...
        async restoreLegacy(backup) {
            if (backup)
                await rename(backup, legacyPath);
            await runProcess("systemctl", ["--user", "daemon-reload"]);
        ...
        async findLegacyBackup() {
            const matches = (await readdir(directory).catch(() => []))
                .filter((name) => name.startsWith(`${PRODUCT.services.linux.legacy}.pre-knot-`) && name.endsWith(".bak"))
                .sort();
            if (matches.length > 1)
...
function launchdMigrationManager(home) {
    const directory = join(home, "Library", "LaunchAgents");
    const legacyPath = join(directory, `${PRODUCT.services.darwin.legacy}.plist`);
    const domain = launchdDomain();
    return {
        async inspect(generation) {
            const identity = generation === "aag" ? PRODUCT.services.darwin.legacy : PRODUCT.services.darwin.current;
            ...
            const disabled = new RegExp(`"${identity.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"\\s*=>\\s*(?:true|disabled)`, "u").test(stdout);
        ...
        async stopAndDisableCurrent() {
            ...
            if (await launchdJobIsLoaded(target))
                await runProcess("/bin/launchctl", ["bootout", target]).catch(() => undefined);
            await unlink(join(directory, `${PRODUCT.services.darwin.current}.plist`)).catch(() => undefined);
        },
        async restoreLegacy(backup) {
            if (backup)
                await rename(backup, legacyPath);
        ...
        async findLegacyBackup() {
            const matches = (await readdir(directory).catch(() => []))
                .filter((name) => name.startsWith(`${PRODUCT.services.darwin.legacy}.plist.pre-knot-`) &&
                name.endsWith(".bak"))
                .sort();
            if (matches.length > 1)
...
async function installSystemdService(configPath) {
    ...
    if (existing?.generation === "aag")
        throw new Error(`Existing AAG service ${existing.identity} is already installed; manage it in place with \`knot service\`. Service migration arrives in a later release.`);
...
async function launchdCommand(command) {
    ...
    if (command === "logs") {
        await access(plistPath, constants.R_OK).catch(() => {
            throw new Error("Knot launch agent is not installed; run `knot service install --config <path>` first");
        });
...
export async function resolveInstalledService(platform, home) {
    ...
    if (installedServices.length > 1)
        throw new Error("Both AAG and Knot service definitions exist; refusing to choose one. Disable or remove one definition before managing the service.");
