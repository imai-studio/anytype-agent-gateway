...
export class Gateway {
    ...
    stop() { this.abort.abort(); this.resolveTerminal(); }
    addRoute(route) {
        ...
        while (!this.abort.signal.aborted) {
            ...
            try {
                ...
                for (;;) {
                    ...
                    if (!catchup.length || cursor === previousCursor)
                        break;
                }
        ...
        while (!this.abort.signal.aborted) {
            try {
                ...
                for (let index = 0; index < candidates.length; index += 10) {
                    ...
                    if (failures.length)
                        throw new Error(`Discussion resolution failed for ${failures.map(item => item.objectId).join(", ")}: ${failures.map(item => item.error).join("; ")}`);
