...
export class CloudCommandStore {
    ...
    persistClaim(command, now = Date.now()) {
        const parsed = commandEnvelopeSchema.parse(command);
        const envelopeJson = canonicalJson(parsed);
        const envelopeDigest = digest("knot.cloud.command.immutable.v1", canonicalJson(immutableCommand(parsed)));
        ...
        const effectKey = digest("knot.cloud.command.effect.v1", parsed.connectorId, parsed.commandId, canonicalJson(parsed.payload));
        this.store.db.exec("BEGIN IMMEDIATE");
        try {
            this.store.db
                .prepare(`INSERT INTO cloud_command_inbox(
           ...
           ON CONFLICT(command_id) DO NOTHING`)
            ...
            if (row.attempt !== parsed.attempt ||
                ...
                this.store.db
                    .prepare(`UPDATE cloud_command_inbox SET envelope_json=?,attempt=?,lease_token=?,lease_token_digest=?,
             lease_expires_at=?,updated_at=? WHERE command_id=?`)
                    .run(envelopeJson, parsed.attempt, parsed.leaseToken, leaseTokenDigest, parsed.leaseExpiresAt * 1_000, now, parsed.commandId);
            }
            this.store.db.exec("COMMIT");
    ...
    list(limit = 100) {
        return this.store.db
            .prepare("SELECT * FROM cloud_command_inbox ORDER BY created_at DESC,command_id DESC LIMIT ?")
            .all(Math.max(1, Math.min(limit, 500))).map(mapCommand);
    }
    recoverInterruptedEffects(now = Date.now()) {
        this.store.db.exec("BEGIN IMMEDIATE");
        try {
            const rows = this.store.db
                .prepare(`SELECT i.command_id,i.effect_key FROM cloud_command_inbox i
           ...
           WHERE i.state='running' AND r.state='running'`)
            ...
            for (const row of rows) {
                ...
                this.store.db
                    .prepare(`UPDATE cloud_effect_receipts SET state='outcome_unknown',
             ...
             completed_at=?,updated_at=? WHERE effect_key=? AND state='running'`)
                ...
                this.store.db
                    .prepare(`UPDATE cloud_command_inbox SET state='dead_letter',
             result_json=?,
    ...
    reject(commandId, reasonCode, now = Date.now()) {
        return this.setTerminalPending(commandId, { outcome: "rejected-by-local-policy", reasonCode: safeCode(reasonCode) }, now, ["received", "awaiting_approval", "queued"]);
    }
    cancel(commandId, now = Date.now()) {
    ...
    retry(commandId, now = Date.now()) {
        ...
        if (receipt && !["failed"].includes(receipt.state))
            throw new Error("This command may have produced an external effect and cannot be retried safely");
        this.store.db
            .prepare(`UPDATE cloud_effect_receipts SET state='prepared',fencing_token=?,error_code=NULL,error=NULL,
        ...
        if (changed)
            this.enqueueProjection(commandId, "retrying", now);
        return changed === 1;
    }
    nextReady(now = Date.now()) {
        const row = this.store.db
            .prepare(`SELECT * FROM cloud_command_inbox WHERE state='queued' AND available_at<=?
         ORDER BY created_at,command_id LIMIT 1`)
            .get(now);
        return row ? mapCommand(row) : undefined;
    }
    startEffect(commandId, now = Date.now()) {
        const row = this.row(commandId);
        if (!row || row.state !== "queued")
            throw new Error("Cloud command is not ready");
        const operationDigest = digest("knot.cloud.command.operation.v1", canonicalJson(this.envelope(commandId).payload));
        const fencingToken = randomUUID();
        this.store.db.exec("BEGIN IMMEDIATE");
        try {
            this.store.db
                .prepare(`INSERT INTO cloud_effect_receipts(
            ...
            const receipt = this.store.db
                .prepare("SELECT operation_digest,state,fencing_token FROM cloud_effect_receipts WHERE effect_key=?")
                .get(row.effect_key);
            if (receipt.operation_digest !== operationDigest)
                throw new Error("Effect key is bound to different operation content");
            if (receipt.state === "succeeded")
                throw new Error("Effect already completed");
            ...
            if (receipt.state === "failed")
                this.store.db
                    .prepare(`UPDATE cloud_effect_receipts SET state='prepared',fencing_token=?,error_code=NULL,
             error=NULL,started_at=NULL,completed_at=NULL,updated_at=? WHERE effect_key=? AND state='failed'`)
                    .run(token, now, row.effect_key);
            const changed = this.store.db
                .prepare(`UPDATE cloud_command_inbox SET state='running',local_attempts=local_attempts+1,
           updated_at=? WHERE command_id=? AND state='queued'`)
                .run(now, commandId).changes;
            if (changed !== 1)
                throw new Error("Cloud command lost its local queue lease");
            this.store.db
                .prepare(`UPDATE cloud_effect_receipts SET state='running',started_at=?,updated_at=?
    ...
    completeEffect(commandId, fencingToken, result, now = Date.now()) {
        ...
        try {
            const receiptChanged = this.store.db
                .prepare(`UPDATE cloud_effect_receipts SET state='succeeded',result_json=?,completed_at=?,updated_at=?
    ...
    failEffect(commandId, fencingToken, input, maximumAttempts = 3, now = Date.now()) {
        const row = this.row(commandId);
        if (!row)
            return false;
        ...
        try {
            const changed = this.store.db
                .prepare(`UPDATE cloud_effect_receipts SET state='failed',error_code=?,error=?,completed_at=?,updated_at=?
           WHERE effect_key=? AND state='running' AND fencing_token=?`)
                .run(safeCode(input.code), input.message.slice(0, 2_000), now, now, row.effect_key, fencingToken).changes;
            if (changed !== 1) {
                this.store.db.exec("COMMIT");
                return false;
            }
            if (retry)
                this.store.db
                    .prepare(`UPDATE cloud_command_inbox SET state='queued',available_at=?,last_error_code=?,
             last_error=?,updated_at=? WHERE command_id=? AND state='running'`)
                    .run(input.retryAt ?? now + backoff(row.local_attempts), safeCode(input.code), input.message.slice(0, 2_000), now, commandId);
            else {
                const result = commandResultSchema.parse({
                    outcome: "failed",
                    retryable: false,
                ...
                this.store.db
                    .prepare(`UPDATE cloud_command_inbox SET state='terminal_pending',result_json=?,
             last_error_code=?,last_error=?,updated_at=? WHERE command_id=? AND state='running'`)
                    .run(JSON.stringify(result), safeCode(input.code), input.message.slice(0, 2_000), now, commandId);
            }
    ...
    updateLease(commandId, leaseExpiresAtSeconds, now = Date.now()) {
        this.store.db
    ...
    expire(now = Date.now()) {
        const rows = this.store.db
            .prepare(`SELECT command_id FROM cloud_command_inbox WHERE expires_at<=?
         AND state IN ('received','awaiting_approval','queued')`)
            .all(now);
        for (const row of rows)
            this.reject(row.command_id, "command-expired", now);
        return rows.length;
    }
    claimProjection(workerId, now = Date.now()) {
        this.store.db.exec("BEGIN IMMEDIATE");
        try {
            this.store.db
                .prepare(`UPDATE cloud_projection_outbox SET state='retrying',lease_owner=NULL,lease_expires_at=NULL,
           available_at=?,updated_at=? WHERE state='in_flight' AND lease_expires_at<=?`)
                .run(now, now, now);
            const row = this.store.db
                .prepare(`SELECT projection_id FROM cloud_projection_outbox
           ...
           ORDER BY created_at,projection_id LIMIT 1`)
            ...
            this.store.db
                .prepare(`UPDATE cloud_projection_outbox SET state='in_flight',attempt=attempt+1,
    ...
    completeProjection(projectionId, workerId, messageId, now = Date.now()) {
        this.store.db
            .prepare(`UPDATE cloud_projection_outbox SET state='delivered',target_message_id=?,delivered_at=?,
         lease_owner=NULL,lease_expires_at=NULL,updated_at=?
         WHERE projection_id=? AND state='in_flight' AND lease_owner=?`)
            .run(messageId, now, now, projectionId, workerId);
    }
    failProjection(projectionId, workerId, error, attempt, now = Date.now()) {
        const terminal = attempt >= 5;
        this.store.db
            .prepare(`UPDATE cloud_projection_outbox SET state=?,available_at=?,last_error=?,
         lease_owner=NULL,lease_expires_at=NULL,updated_at=?
         WHERE projection_id=? AND state='in_flight' AND lease_owner=?`)
            .run(terminal ? "dead_letter" : "retrying", terminal ? now : now + backoff(attempt), error.slice(0, 2_000), now, projectionId, workerId);
    }
    setTerminalPending(commandId, result, now, allowed) {
        const parsed = commandResultSchema.parse(result);
    ...
    enqueueProjection(commandId, state, now) {
        const originEffectKey = digest("knot.cloud.projection.v1", commandId, state);
        this.store.db
            .prepare(`INSERT INTO cloud_projection_outbox(
           projection_id,command_id,origin_effect_key,payload_json,state,available_at,created_at,updated_at
            ...
            .run(randomUUID(), commandId, originEffectKey, JSON.stringify({ commandId, state, originEffectKey }), now, now, now);
    }
...
export class CloudWorkflowExtension {
    client;
    executor;
    config;
    anytype;
    ...
    async tick(signal, executionSignal) {
        ...
        if (!this.recovered) {
            ...
            if (recovered)
                this.log("cloud_command_effects_quarantined", { count: recovered });
            this.recovered = true;
        }
    ...
    async execute(record, now, signal) {
        ...
        try {
            ...
            }, this.config.maximumLocalAttempts, this.now());
        }
        finally {
            heartbeat.abort();
            await leaseTask;
    ...
    async maintainLease(commandId, signal) {
    ...
    async submitTerminalResults(now, signal) {
        for (const record of this.inbox.terminalPending(this.lastResultCommandId, now)) {
            ...
            try {
                ...
                if (shouldDeferCloudRequests(error, signal))
                    throw error;
                this.inbox.failSubmission(command, record.result, error instanceof CloudRequestError && error.options.status !== undefined
...
export class AnytypeCloudCommandExecutor {
    ...
    async execute(command, effectKey, signal) {
        ...
        const provenance = (spaceId, objectId, messageId) => ({
            ...
            case "chat.read": {
                ...
                return success({
                    ...
                    messages: messages.map((message) => {
                        const participantDigest = digest("knot.anytype.participant.v1", String(message.creator ?? "unknown"));
                        return {
...
function localPolicyDenial(command, config, now) {
    ...
    if (!config.allowedScopes.includes(command.requiredScope))
        return "scope-denied";
...
function approvalRequired(command, config) {
    if (config.approval === "all")
        return true;
    if (config.approval === "none")
        return false;
    return !["object.read", "object.query", "collection.read", "file.download", "chat.read"].includes(command.payload.operation.type);
}
function isRetrySafe(command) {
    return (command.payload.domain === "publication" ||
        ["object.read", "object.query", "collection.read", "file.download", "chat.read"].includes(command.payload.operation.type));
}
...
function unsupported(type) {
    return new CloudEffectError(`The local Anytype adapter does not support ${type}`, "unsupported-operation", false);
}
function cloudExecutionError(error) {
...
function snapshot(raw, spaceId, objectId, provenance) {
    const rawType = raw.type;
    const typeKey = typeof raw.type_key === "string"
        ? raw.type_key
...
async function resolveAsset(digestValue, name, roots) {
    ...
    throw new CloudEffectError("No allowed local asset matches the requested digest", "asset-unavailable", false);
}
function stringField(value, ...keys) {
    for (const key of keys)
        if (typeof value[key] === "string" && value[key])
            return value[key];
    throw new CloudEffectError("Anytype returned an object without an expected ID", "invalid-anytype-response", false);
}
...
function immutableCommand(command) {
    return {
