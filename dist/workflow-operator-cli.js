...
import { canonicalJson, workflowPrincipalDigest } from "./automation/workflow.js";
import { Store } from "./store.js";
export async function workflowList(input) {
...
export async function workflowShow(input) {
    await withOperator(input.agentConfigFile, ({ store, queue, config }) => {
...
export async function workflowApprovalAction(input) {
    await withOperator(input.agentConfigFile, ({ store, config }) => {
        ...
        const decision = input.action === "approve" ? "approved" : input.action === "reject" ? "rejected" : "revoked";
        const reasonCode = input.action === "approve" ? input.reasonCode : requireReasonCode(input.reasonCode);
...
export async function workflowSetEnabled(input) {
    await withOperator(input.agentConfigFile, ({ store, config }) => {
        requireConfirmed(input.yes);
        const actorPrincipalDigest = authorizedActor(config, input.actorDigest);
        ...
        (input.output ?? console.log)(`${result.enabled ? "enabled" : "disabled"}: ${result.workflowId}`);
    });
}
export async function workflowManualRun(input) {
    await withOperator(input.agentConfigFile, ({ store, config }) => {
        requireConfirmed(input.yes);
        const actorPrincipalDigest = authorizedActor(config, input.actorDigest);
...
export async function workflowRunList(input) {
    await withOperator(input.agentConfigFile, ({ queue }) => {
        const records = queue.runs(input.limit ?? 100, input.state).map(safeRun);
        emit(input, records, (record) => [record.runId, record.workflowId, record.state, record.errorDigest ?? ""].join("\t"));
    });
}
export async function workflowRunShow(input) {
    await withOperator(input.agentConfigFile, ({ queue }) => {
        ...
        const record = {
            ...
            steps: queue.steps(input.runId).map((step) => ({
                ...
                resultDigest: step.result === undefined ? undefined : sensitiveDigest("step-result", step.result),
                errorDigest: step.error === undefined ? undefined : sensitiveDigest("step-error", step.error),
                updatedAt: step.updatedAt,
            })),
            attempts: queue.steps(input.runId).flatMap((step) => queue.attempts(input.runId, step.stepId).map((attempt) => ({
                attemptId: attempt.attemptId,
                stepId: attempt.stepId,
                attemptNumber: attempt.attemptNumber,
                state: attempt.state,
                errorDigest: attempt.error === undefined
                    ? undefined
...
export async function workflowRunMutation(input) {
    await withOperator(input.agentConfigFile, ({ queue, config }) => {
        ...
        if (!run)
            throw new Error("Unknown workflow run");
        const now = input.now ?? Date.now();
        const changed = input.action === "cancel"
            ...
            : queue.retryRun(input.runId, workflowAuthorityHash(config.automation), actorPrincipalDigest, reasonCode, randomUUID(), now);
        if (!changed)
            throw new Error(input.action === "retry"
...
export async function workflowEventList(input) {
    await withOperator(input.agentConfigFile, ({ queue }) => {
        const records = queue.recentEvents(input.limit ?? 100).map((event) => ({
            ...
            diffDigest: event.diff === undefined
                ? undefined
                : sensitiveDigest("event-diff", event.diff),
            recordedAt: event.recordedAt,
        }));
        emit(input, records, (record) => [record.eventId, record.kind, record.source, record.payloadDigest].join("\t"));
    });
}
export async function workflowAuditList(input) {
    await withOperator(input.agentConfigFile, ({ store }) => {
        ...
        emit(input, records, (record) => [String(record.sequence), record.action, record.workflowId ?? "", record.runId ?? ""].join("\t"));
    });
}
export async function workflowDeadLetterList(input) {
    await withOperator(input.agentConfigFile, ({ queue }) => {
        const limit = input.limit ?? 100;
        const records = [
            ...queue.runs(limit, "dead_letter").map((run) => ({ type: "run", ...safeRun(run) })),
            ...
            .slice(0, limit);
        emit(input, records, (record) => record.type === "run"
...
async function withOperator(configFile, operation) {
...
function authorizedActor(config, actorDigest) {
...
function requireConfirmed(confirmed) {
    if (!confirmed)
        throw new Error("Mutation requires explicit --yes confirmation");
}
...
function safeRun(run) {
    return {
        ...
        cancelReasonDigest: run.cancelReason === undefined
            ? undefined
...
function sensitiveDigest(domain, value) {
...
function emit(input, records, line) {
