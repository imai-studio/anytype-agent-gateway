...
export const cloudConfigSchema = z
    ...
    .strict();
export function resolveCloudPaths(options = {}) {
...
export async function initializeCloudConfig(input) {
    ...
    if (!publicJwk.x)
        throw new Error("Node did not export the Ed25519 public key");
...
export async function saveCloudConfig(paths, config) {
    await ensurePrivateDirectory(dirname(paths.configFile));
    await atomicPrivateWrite(paths.configFile, `${JSON.stringify(cloudConfigSchema.parse(config), null, 2)}\n`);
}
export async function savePairingCredentials(paths, credentials) {
    await ensurePrivateDirectory(dirname(paths.pairingFile));
    await atomicPrivateWrite(paths.pairingFile, `${JSON.stringify(pairingCredentialsSchema.parse(credentials), null, 2)}\n`);
}
export async function loadPairingCredentials(paths) {
    const raw = await readFile(paths.pairingFile, "utf8").catch((error) => {
        if (isMissingFile(error))
...
export async function removePairingCredentials(paths) {
...
export async function cloudFileMode(path) {
