    ...
    async start(input, onEvent) {
            ...
            try {
                ...
                if (savedSessionId && initialized.agentCapabilities?.loadSession) {
                    ...
                    try {
                        ...
                        if (!savedSessionUnavailable(error) && !savedSessionInternallyBroken(error))
                            throw error;
        ...
        return {
            ...
            steer: async (message) => {
                ...
                if (!acceptingSteers)
                    throw new Error("Codex ACP turn already completed before the steer arrived");
...
function savedSessionUnavailable(error) {
    if (error instanceof acp.RequestError && error.code === -32002)
        return true;
