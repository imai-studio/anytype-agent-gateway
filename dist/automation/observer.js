...
export class WorkflowObserver {
    ...
    async scanSpaceOnce(spaceId) {
        ...
        try {
            ...
            const interval = changes > 0 || !complete
                ? minimum
    ...
    observeObject(spaceId, object, observedAt) {
        ...
        try {
            ...
            if (parsed !== undefined) {
                ...
                if (result.success)
                    ...
                    errors.push(...result.error.issues.map((issue) => `${issue.path.length ? issue.path.join(".") : "definition"}: ${issue.message}`));
            }
        }
        catch (error) {
        ...
        if (!principal)
            errors.push("Workflow editor identity is not verified");
        let version;
        if (definition) {
            const policy = evaluateWorkflowPolicy(definition, { sourceSpaceId: spaceId });
            if (policy.missingCapabilities.length)
                errors.push(`Workflow omits required capabilities: ${policy.missingCapabilities.join(", ")}`);
    ...
    archiveMissing(spaceId, startedAt, observedAt) {
        ...
        for (const definition of archived) {
            ...
            this.recordEvent("object.archived", spaceId, {
...
function extractWorkflowSource(source) {
    if (!source)
        throw new Error("Workflow object has no source body");
    if (source.length > 1_000_000)
        throw new Error("Workflow source exceeds 1000000 characters");
