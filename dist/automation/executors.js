...
import { anytypeMaterializeConfigSchema, anytypeQueryConfigSchema, anytypeReadConfigSchema, anytypeUpsertConfigSchema, anytypeWriteConfigSchema, agentConfigSchema, canonicalJson, jsonValueSchema, notifyConfigSchema, transformConfigSchema, workflowApprovalHash, } from "./workflow.js";
import { workflowAuthorityHash } from "./policy.js";
/** Executes only the closed, typed local workflow catalog. */
export class TypedWorkflowStepExecutor {
    store;
    config;
    anytype;
    runtime;
    ...
    receipts;
    upsertTails = new Map();
    constructor(store, config, anytype, runtime, fallback) {
        this.store = store;
        this.config = config;
    ...
    async execute(claim, definition, signal) {
        ...
        if (!step)
            return failure("Workflow step no longer exists");
        if (signal.aborted)
            throw signal.reason;
        try {
            ...
            switch (step.kind) {
                ...
                case "anytype.write":
                    ...
                    return await this.effect(claim, definition, step, signal, () => this.write(claim, step.config, signal));
                case "anytype.upsert":
                    return await this.upsert(claim, definition, step, signal);
                case "anytype.materialize":
                    this.validateMaterialize(claim, step.config);
                    return await this.effect(claim, definition, step, signal, () => this.materialize(claim, step.config, signal));
                case "transform":
                    return this.transform(claim, step.config);
                case "notify":
                    this.validateNotify(claim, step.config);
                    return await this.effect(claim, definition, step, signal, () => this.notify(claim, step.config, signal));
                case "agent":
                    this.validateAgent(step.config);
                    return await this.effect(claim, definition, step, signal, () => this.invokeAgent(claim, step.config, signal));
                default:
                    return await this.fallback.execute(claim, definition, signal);
            }
        }
        catch (error) {
            if (error instanceof ClosedExecutorError)
                return failure(error.message);
            throw error;
        }
    }
    validateWrite(claim, raw) {
        const parsed = anytypeWriteConfigSchema.safeParse(raw);
        if (!parsed.success)
            throw new ClosedExecutorError("anytype.write configuration is invalid");
        ...
        if (input.operation !== "create" && !objectId)
            throw new ClosedExecutorError(`anytype.${input.operation} requires objectId or an object event`);
    }
    validateMaterialize(claim, raw) {
        ...
        if (!objectIds.size)
            throw new ClosedExecutorError("Materialize resolved no object IDs");
        if (objectIds.size > 100)
            throw new ClosedExecutorError("Materialize exceeds 100 objects");
    }
    validateNotify(claim, raw) {
        const parsed = notifyConfigSchema.safeParse(raw);
        if (!parsed.success)
            throw new ClosedExecutorError("notify configuration is invalid");
        ...
        if (parsed.data.inputStepId)
            this.inputResult(claim, parsed.data.inputStepId);
    }
    validateAgent(raw) {
        const parsed = agentConfigSchema.safeParse(raw);
        if (!parsed.success)
            throw new ClosedExecutorError("agent configuration is invalid");
        if (this.config.runtime.kind === "openclaw")
            throw new ClosedExecutorError("agent workflow invocation requires a capability-scoped runtime; OpenClaw workflow mode is unavailable");
        const project = parsed.data.project ?? this.config.runtime.defaultProject;
        if (!project)
            throw new ClosedExecutorError("agent workflow invocation requires an explicitly authorized project");
    ...
    async read(claim, raw, signal) {
        const parsed = anytypeReadConfigSchema.safeParse(raw ?? {});
        if (!parsed.success)
            return failure("anytype.read configuration is invalid");
        ...
        if (!objectId)
            return failure("anytype.read requires objectId or an object event");
        const object = await this.anytype.getObject(spaceId, objectId, signal);
        return success(toBoundedJson({ spaceId, objectId, object }));
    }
    async query(claim, raw, signal) {
        const parsed = anytypeQueryConfigSchema.safeParse(raw ?? {});
        if (!parsed.success)
            return failure("anytype.query configuration is invalid");
        if (!this.anytype.searchSpace)
            return failure("Anytype query is not supported locally");
        const event = this.sourceEvent(claim);
        const input = parsed.data;
        const spaceId = input.spaceId ?? event.spaceId;
        this.assertSpaceAuthorized(spaceId);
        const objects = await this.anytype.searchSpace(spaceId, {
            query: input.text,
    ...
    async write(claim, raw, signal) {
        ...
        if (input.operation === "create") {
            if (!this.anytype.createObject)
                throw new ClosedExecutorError("Anytype create is unavailable");
            const object = await this.anytype.createObject(spaceId, {
        ...
        const object = await this.anytype.updateObject(spaceId, objectId, {
            ...(input.values.name !== undefined ? { name: input.values.name } : {}),
    ...
    async upsert(claim, definition, step, signal) {
        const parsed = anytypeUpsertConfigSchema.safeParse(step.config);
        if (!parsed.success)
            return failure("anytype.upsert configuration is invalid");
        ...
        return this.withUpsertLock(lockKey, async () => {
            ...
            for (let page = 0; page < 10; page += 1) {
                const candidates = await this.anytype.searchSpace(spaceId, { query: input.matchName, types: [input.typeKey], offset: page * 100, limit: 100 }, signal);
                ...
                matches.push(...named);
                if (matches.length > 1)
                    return failure("anytype.upsert found more than one exact match");
            ...
            return this.effect(claim, definition, step, signal, async () => {
                if (selectedId) {
                    const object = await this.anytype.updateObject(spaceId, selectedId, {
    ...
    async materialize(claim, raw, signal) {
        const parsed = anytypeMaterializeConfigSchema.safeParse(raw);
        if (!parsed.success)
            throw new ClosedExecutorError("anytype.materialize configuration is invalid");
        if (!this.anytype.addObjectsToList)
    ...
    async notify(claim, raw, signal) {
        ...
        const text = [input.message, derived]
            ...
            .join("\n");
        const messageId = await this.anytype.sendMessage(target.spaceId, target.chatId, { text }, signal);
        return { operation: "notified", connectionRef: input.connectionRef, messageId };
    }
    async invokeAgent(claim, raw, signal) {
    ...
    async effect(claim, definition, step, signal, execute, discriminator = null) {
        const preflight = this.preflight(claim, definition, signal);
        if (preflight)
            return preflight;
        const start = this.receipts.begin(claim, {
            kind: step.kind,
            config: (step.config ?? {}),
        ...
        if (start.kind === "replay")
            return success(start.result);
        if (start.kind === "blocked")
            return failure(start.error);
        const finalPreflight = this.preflight(claim, definition, signal);
        if (finalPreflight) {
            this.receipts.failBeforeEffect(start.effectKey, claim.attempt.fencingToken, "Effect authority changed");
        ...
        try {
            ...
            this.receipts.outcomeUnknown(start.effectKey, claim.attempt.fencingToken, safeEffectError(error));
            if (signal.aborted)
                throw signal.reason;
            return failure(error instanceof ClosedExecutorError
                ? error.message
                : "External effect outcome is unknown; Knot will not repeat it automatically");
        }
    }
    preflight(claim, definition, signal) {
        if (signal.aborted)
            throw signal.reason;
        ...
        if (!this.store.currentWorkflowApproval(claim.run.workflowId, claim.run.approvalHash, claim.run.authorityHash))
    ...
    sourceEvent(claim) {
        const row = this.store.db
            .prepare(`SELECT e.space_id,e.object_id FROM workflow_deliveries d
         JOIN normalized_events e ON e.event_id=d.event_id WHERE d.delivery_id=?`)
            .get(claim.run.deliveryId);
        if (!row)
            throw new ClosedExecutorError("Workflow source event is unavailable");
        return { spaceId: row.space_id, ...(row.object_id ? { objectId: row.object_id } : {}) };
    }
    inputResult(claim, stepId) {
    ...
    assertEventSpaceApproval(claim, definition) {
        ...
        const version = this.store.db
            ...
            .get(claim.run.workflowId, claim.run.versionHash);
        if (version &&
            version.space_id !== eventSpaceId &&
            !definition.spec.capabilities.includes("anytype.cross-space"))
            throw new ClosedExecutorError(`Workflow event crossed from ${version.space_id} to ${eventSpaceId} without anytype.cross-space approval`);
    }
    async withUpsertLock(key, operation) {
        ...
        try {
            ...
            release();
            if (this.upsertTails.get(key) === tail)
                this.upsertTails.delete(key);
        }
    }
}
class WorkflowEffectReceipts {
    ...
    recoverInterruptedEffects(now = Date.now()) {
        const rows = this.store.db
            .prepare("SELECT effect_key,run_id,state FROM workflow_effect_receipts WHERE state IN ('running','outcome_unknown')")
            .all();
        for (const row of rows) {
            if (row.state === "running")
                this.store.db
                    .prepare(`UPDATE workflow_effect_receipts SET state='outcome_unknown',
            ...
            this.queue.deadLetterRun(row.run_id, "External effect outcome is unknown; Knot will not repeat it automatically", now);
        }
        return rows.length;
    }
    begin(claim, operation, maximumEffectsPerRun, now = Date.now()) {
        ...
        try {
            ...
            this.store.db
                .prepare(`INSERT INTO workflow_effect_receipts(
             effect_key,run_id,step_id,operation_digest,state,fencing_token,updated_at
                ...
                .run(effectKey, claim.run.runId, claim.step.stepId, operationDigest, claim.attempt.fencingToken, now);
            const row = this.store.db
                .prepare("SELECT operation_digest,state,result_json FROM workflow_effect_receipts WHERE effect_key=?")
                .get(effectKey);
            ...
            const changed = this.store.db
                .prepare(`UPDATE workflow_effect_receipts SET state='running',fencing_token=?,started_at=?,
           ...
           WHERE effect_key=? AND state IN ('prepared','failed')`)
            ...
            if (changed !== 1)
                throw new Error("Workflow effect receipt could not be started");
            this.store.db.exec("COMMIT");
    ...
    complete(effectKey, fencingToken, result, now = Date.now()) {
        return (this.store.db
            .prepare(`UPDATE workflow_effect_receipts SET state='succeeded',result_json=?,completed_at=?,updated_at=?
           WHERE effect_key=? AND state='running' AND fencing_token=?`)
            .run(canonicalJson(result), now, now, effectKey, fencingToken).changes === 1);
    }
    failBeforeEffect(effectKey, fencingToken, error, now = Date.now()) {
        this.store.db
            .prepare(`UPDATE workflow_effect_receipts SET state='failed',error=?,completed_at=?,updated_at=?
         WHERE effect_key=? AND state='running' AND fencing_token=?`)
            .run(safeEffectError(error), now, now, effectKey, fencingToken);
    }
    outcomeUnknown(effectKey, fencingToken, error, now = Date.now()) {
        this.store.db
            .prepare(`UPDATE workflow_effect_receipts SET state='outcome_unknown',error=?,completed_at=?,updated_at=?
         WHERE effect_key=? AND state='running' AND fencing_token=?`)
            .run(safeEffectError(error), now, now, effectKey, fencingToken);
    }
}
...
function toBoundedJson(value, maximumBytes = 1024 * 1024) {
...
function objectIdOf(value, fallback) {
    const id = typeof value.id === "string" ? value.id : fallback;
    if (!id)
        throw new ClosedExecutorError("Anytype returned no object ID");
    return id;
}
function typeKeyOf(value) {
    if (typeof value.type_key === "string")
        return value.type_key;
    if (typeof value.type === "string")
        return value.type;
    if (value.type && typeof value.type === "object" && "key" in value.type)
        return String(value.type.key);
    return undefined;
}
function objectIdsFrom(value) {
    if (Array.isArray(value))
        return value.flatMap(objectIdsFrom);
    ...
    if (Array.isArray(value.objectIds))
        return value.objectIds.filter((entry) => typeof entry === "string");
    if (Array.isArray(value.objects))
        return value.objects.flatMap(objectIdsFrom);
    return [];
}
...
function safeEffectError(error) {
    const raw = error instanceof Error ? error.message : String(error);
    return raw
        .replace(/\bBearer\s+[A-Za-z0-9._~+/=-]+/giu, "Bearer [redacted]")
        .replace(/["']?\b(api[_ -]?key|authorization|credential|message|password|prompt|secret|token)\b["']?\s*[:=]\s*["']?[^\s,"';}]+["']?/giu, "$1=[redacted]")
        .slice(0, 2_000);
}
