...
program
    ...
    .option("-c, --config <path>", "configuration file (defaults to an existing AAG path, then ~/.config/knot/agent.yaml)")
    .action(async (options) => {
...
program
    ...
    console.log(`ok: migration compatibility (config=${legacyConfig ? "legacy" : "knot"}, state=${legacyState ? "legacy" : "knot"})`);
    ...
    if (config.automation.enabled && config.automation.observation) {
        ...
        for (const spaceId of config.automation.allowedSpaceIds) {
            const definitions = await anytype.searchWorkflowObjects(spaceId, config.automation.definitionTypeKeys, 0, 10);
            visible += definitions.length;
            verifiedEditors += definitions.filter((definition) => !definition.observationError && definition.editorParticipantId).length;
            authorizedEditors += definitions.filter((definition) => !definition.observationError &&
                definition.editorParticipantId &&
                config.automation.allowedAuthorIds.includes(definition.editorParticipantId)).length;
        }
        console.log(`ok: read-only workflow observer (${config.automation.allowedSpaceIds.length} spaces, ${config.automation.definitionTypeKeys.length} type keys, ${visible} visible sample definitions, ${verifiedEditors} with native editor identity)`);
        if (visible > 0 && verifiedEditors === 0)
            console.warn("warning: Anytype did not expose the last_modified_by system object property; workflow definitions will remain invalid");
...
program
    ...
    .option("--route-id <id>")
    .option("--space-id <id>")
    .action(async (options) => runMcpServer(selectedConfigPath(options.config), {
...
cloud
    ...
    .option("--config <path>", "cloud configuration file")
    .action(async (options) => cloudDoctor({ ...(options.config ? { configFile: options.config } : {}) }));
cloud
    .command("revoke")
    ...
    .option("--config <path>", "cloud configuration file")
    .action(async (options) => cloudRevoke({
...
cloudOperation
    ...
    console.log(JSON.stringify(await publicationOperationStatus({
        operationId,
        ...(options.config ? { configFile: options.config } : {}),
    }), null, 2));
});
cloudOperation
    ...
    .option("--config <path>", "cloud configuration file")
    .action(async (operationId, options) => {
    console.log(JSON.stringify(await retryPublicationOperation({
        operationId,
...
publish
    ...
    const prepared = options.assetManifest
        ...
        : undefined;
    console.log(JSON.stringify(await publicationAction({
        action: "push",
        siteId: options.site,
...
publishAssets
    ...
    console.log(JSON.stringify(await preparePublicationAssetManifest({
        manifestPath,
        ...(options.config ? { configFile: options.config } : {}),
    }), null, 2));
});
...
function makeRuntime(config, store, configPath) {
    ...
    const mcpServer = (config.tools.anytype.enabled || config.tools.publish.enabled) && configPath
        ? {
...
function managementCommand(config, routeId, capabilities) {
    ...
    if (config.tools.publish.enabled)
        commands.push(`Publish tool: aag_publish with one configured typed action${capabilities?.publish ? ` and actor_capability=${JSON.stringify(capabilities.publish)}` : ""}`);
