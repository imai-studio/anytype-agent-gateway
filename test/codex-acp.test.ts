...
describe("Codex ACP continuity", () => {
  it("persists a session, loads it after reopening SQLite, and ignores replayed history", async () => {
    ...
    const first = driver(firstStore, { FAKE_ACP_LOG: firstLog, FAKE_ACP_NEW_SESSION_ID: "durable-session" });
    expect(await (await first.start({ sessionKey: "aag:thread", prompt: "first" }, () => undefined)).result).toMatchObject({ text: "fresh reply" });
    ...
    const result = await (await second.start({ sessionKey: "aag:thread", prompt: "second" }, event => events.push(event))).result;
    ...
    expect(calls.find(call => call.method === "session/load")?.params).toMatchObject({ sessionId: "durable-session" });
  ...
  it("replaces a saved session only when the agent reports it unavailable", async () => {
    ...
    const runtime = driver(store, { FAKE_ACP_LOG: logPath, FAKE_ACP_LOAD_ERROR: "missing", FAKE_ACP_NEW_SESSION_ID: "replacement-session" });
    await (await runtime.start({ sessionKey: "aag:thread", prompt: "continue" }, () => undefined)).result;
    expect(store.codexAcpSession("aag:thread")).toBe("replacement-session");
    expect((await log(logPath)).filter(call => call.method === "session/load" || call.method === "session/new").map(call => call.method)).toEqual(["session/load", "session/new"]);
    store.close();
  });

  it("replaces a saved session when loading it fails with an ACP internal error", async () => {
    const directory = await temporaryDirectory();
    const logPath = join(directory, "calls.jsonl");
    const store = new Store(join(directory, "state.sqlite"));
    store.saveCodexAcpSession("aag:thread", "broken-session");
    const runtime = driver(store, { FAKE_ACP_LOG: logPath, FAKE_ACP_LOAD_ERROR: "internal", FAKE_ACP_NEW_SESSION_ID: "replacement-session" });

    const result = await (await runtime.start({ sessionKey: "aag:thread", prompt: "continue" }, () => undefined)).result;

    expect(result).toMatchObject({ text: "fresh reply" });
    expect(store.codexAcpSession("aag:thread")).toBe("replacement-session");
    expect((await log(logPath)).filter(call => call.method === "session/load" || call.method === "session/new").map(call => call.method)).toEqual(["session/load", "session/load", "session/new"]);
    store.close();
  });
  ...
  it("does not replace a saved session after an unrelated load failure", async () => {
    ...
    await expect(runtime.start({ sessionKey: "aag:thread", prompt: "continue" }, () => undefined)).rejects.toThrow("Authentication required");
  ...
  it("does not replay a failed prompt or discard its persisted session", async () => {
    ...
    const active = await runtime.start({ sessionKey: "aag:thread", prompt: "recover" }, () => undefined);

    await expect(active.result).rejects.toThrow("Internal error");
    expect(store.codexAcpSession("aag:thread")).toBe("poisoned-session");
    const calls = await log(logPath);
    expect(calls.filter(call => call.method === "session/prompt").map(call => call.params.sessionId)).toEqual(["poisoned-session"]);
    store.close();
  });
  ...
  it("preserves the persisted session when an active run is cancelled", async () => {
    const directory = await temporaryDirectory();
    const store = new Store(join(directory, "state.sqlite"));
    const runtime = driver(store, { FAKE_ACP_NEW_SESSION_ID: "cancelled-session", FAKE_ACP_PROMPT_DELAY_MS: "1000" });
    const active = await runtime.start({ sessionKey: "aag:thread", prompt: "cancel me" }, () => undefined);
...
describe("Codex ACP output and steering", () => {
  it("terminates an ACP child that never completes setup", async () => {
    ...
    await expect(runtime.start({ sessionKey: "hung-setup", prompt: "answer" }, () => undefined)).rejects.toThrow("Codex ACP setup timed out");
  });

  it("suppresses the split internal skill warning from deltas and the final result", async () => {
    const warning = "Warning: Skill descriptions were shortened to fit the 2% skills context budget. Codex can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest.";
    const events: RuntimeEvent[] = [];
    const runtime = driver(undefined, { FAKE_ACP_UPDATES: JSON.stringify([
      { sessionUpdate: "agent_message_chunk", messageId: "final", content: { type: "text", text: warning.slice(0, 35) }, _meta: { codex: { phase: "final_answer" } } },
      { sessionUpdate: "agent_message_chunk", messageId: "final", content: { type: "text", text: `${warning.slice(35)}\n\n` }, _meta: { codex: { phase: "final_answer" } } },
      { sessionUpdate: "agent_message_chunk", messageId: "final", content: { type: "text", text: "visible answer" }, _meta: { codex: { phase: "final_answer" } } }
    ]) });
  ...
  it("uses the terminal final-answer message instead of concatenating progress replies", async () => {
    const updates = [
      { sessionUpdate: "agent_message_chunk", messageId: "progress", content: { type: "text", text: "I am working." }, _meta: { codex: { phase: "commentary" } } },
      { sessionUpdate: "agent_message_chunk", messageId: "final", content: { type: "text", text: "CLEAN_FINAL" }, _meta: { codex: { phase: "final_answer" } } }
    ];
    const events: RuntimeEvent[] = [];
...
async function log(path: string): Promise<Array<{ method: string; params: Record<string, unknown> }>> {
