...
describe("protocol boundaries", () => {
  it("parses multiline SSE data and rejects malformed envelopes", () => {
    expect(parseSseBlock("event: message\ndata: {\"type\":\ndata: \"message_added\"}" )?.type).toBe("message_added");
  ...
  it("creates only configured peer marks and enforces fan-out", () => {
    const value = config({ coordination: { peers: [
...
describe("projection ordering", () => {
  ...
  it("does not let a delayed progress edit overwrite the final answer", async () => {
    ...
    const projection = await RunProjection.create(anytype, config({ responses: { mode: "verbose" } }), conversation, "trigger");
    projection.onEvent({ type: "text-delta", text: "partial" });
    await new Promise(resolve => setTimeout(resolve, 950));
    await projection.finish({ text: "final" });
    expect(anytype.messages.find(message => message.id === projection.messageId)?.content?.text).toBe("final");
  });

  it("projects tool milestones before the final answer", async () => {
    const anytype = new FakeAnytype();
    const projection = await RunProjection.create(anytype, config({ responses: { mode: "milestones" } }), conversation, "trigger");
  ...
  it("implements keep and replace silent placeholder policies", async () => {
    ...
    const keepProjection = await RunProjection.create(kept, config({ responses: { silentPlaceholder: "keep" } }), conversation, "trigger");
    ...
    const replaceProjection = await RunProjection.create(replaced, config({ responses: { silentPlaceholder: "replace", silentText: "No reply needed." } }), conversation, "trigger");
  ...
  it("marks truncated final answers and never splits a surrogate pair", async () => {
    const anytype = new FakeAnytype();
    const projection = await RunProjection.create(anytype, config({ responses: { maxCharacters: 100 } }), conversation, "trigger");
