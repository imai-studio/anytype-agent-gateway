...
describe("session persistence", () => {
  it("upgrades a legacy database without losing existing state", () => {
    ...
    expect((store.db.prepare("PRAGMA table_info(handled_message_versions)").all() as Array<{ name: string }>).map(column => column.name)).toContain("fingerprint");
    expect(store.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_bindings'").get()).toBeDefined();
    store.close();
  });
  ...
  it("maps an Anytype thread to exactly one native session and persists capabilities", () => {
    ...
    expect(store.bindingForNativeSession("openclaw", { id: "native-1" })?.threadKey).toBe("discussion:root");
    expect(store.bindingForNativeSession("openclaw", { key: binding.nativeSessionKey })?.routeId).toBe(binding.routeId);

    const reset = store.updateSessionBinding(binding.threadKey, {
      nativeSessionId: "native-2",
      generation: 1,
      eventCursor: null,
      state: "resetting"
    }, 20);
    expect(reset).toMatchObject({ nativeSessionId: "native-2", generation: 1, state: "resetting", updatedAt: 20 });
    expect(reset).not.toHaveProperty("eventCursor");
    expect(reset?.createdAt).toBe(10);

    store.saveRuntimeCapabilities("openclaw", { nativeScheduling: true, sessionEvents: true, protocol: "channel-v1" });
    expect(store.runtimeCapabilities("openclaw")).toEqual({ nativeScheduling: true, sessionEvents: true, protocol: "channel-v1" });
    store.close();
  });
  ...
  it("keeps one open output cycle per thread and advances its durable sequence", () => {
    ...
    expect(() => store.createOutputCycle({ id: "cycle-conflict", threadKey: "thread", anytypeMessageId: "message-conflict" })).toThrow();

    expect(store.updateOutputCycle("cycle-1", { thinkingText: "Considering", answerText: "Done", phase: "answer", eventCursor: "cursor-2" }, 20)).toMatchObject({
      thinkingText: "Considering",
      answerText: "Done",
      phase: "answer",
      eventCursor: "cursor-2"
    });
    expect(store.finishOutputCycle("cycle-1", "complete", 30)).toMatchObject({ state: "complete", completedAt: 30 });

    const second = store.createOutputCycle({ id: "cycle-2", threadKey: "thread", anytypeMessageId: "message-2" }, 40);
    expect(second.sequence).toBe(2);
    expect(store.listOutputCycles("thread").map(cycle => cycle.id)).toEqual(["cycle-1", "cycle-2"]);
    store.close();
  });
});
...
describe("durable outbound delivery", () => {
  it("deduplicates enqueue, leases work, retries failures, and acknowledges delivery", () => {
    ...
    expect(claimed).toMatchObject({ id: "out-1", status: "claimed", claimedBy: "worker-a", attempts: 1, payload: { text: "hello" } });
    expect(store.failOutbound("out-1", "temporary", { workerId: "worker-a", retryAt: 200, now: 120 })).toBe(true);
    expect(store.claimOutbound("worker-b", { now: 199 })).toEqual([]);

    expect(store.claimOutbound("worker-b", { now: 200 })[0]).toMatchObject({ id: "out-1", attempts: 2, claimedBy: "worker-b" });
  ...
  it("reclaims expired leases and records proactive delivery and bridge cursors idempotently", () => {
    ...
    expect(store.claimOutbound("worker-b", { now: 110, leaseMs: 10 })[0]).toMatchObject({ claimedBy: "worker-b", attempts: 2 });
