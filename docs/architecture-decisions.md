...
One running Knot instance maps one Anytype member directly to one runtime-backed agent.
...
Independently taggable agents need separate Anytype identities and isolated Knot processes. This remains true when several processes share one physical host.

## One agent process per machine or service instance

The supported deployment unit is one Knot process, one identity, and one runtime agent. Run the process in the foreground or install it as an operating-system user service.
...
An offline member remains visible and taggable in Anytype; Knot does not post presence or failure messages merely because its process is offline. Starting the command or service brings that identity online again, and reconnect catch-up handles eligible messages that arrived while it was down.
...
This keeps setup reproducible and makes a future `knot init`/installer flow possible without requiring a separate desktop application.
...
Knot is not an agent runtime. OpenClaw, Codex, or a future adapter handles reasoning, memory, compaction, approvals, tools, and model selection.
...
The Heart adapter is optional and has a separate dependency-license boundary. The Any Source Available License 1.0 governs `anytype-heart`. Core Knot remains Apache-2.0; see [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md).
...
Agents need more than prompt context when asked to create a daily object, update a task, search a space, upload a file, add an object to a collection, or return an object link. Knot exposes those operations through a policy-mediated MCP server rather than handing an API key to the model.
...
Project lists tell the runtime which directories to use. Knot passes them as OpenClaw context or ACP working and additional directories. The runtime settings, sandbox, service account, or container must enforce access.
...
Peer agents may talk in the same chat. Outbound coordination uses an explicit peer marker that Knot converts into a real Anytype mention, while inbound peer wakeups remain subject to allowlists, hop limits, fan-out limits, and an activation circuit breaker.

Static chat routes remain the default. A space can explicitly enable chat discovery with a separate wake policy and sender allowlist. Knot then subscribes to every current and newly created chat in that space, while mention-based waking keeps mere membership from invoking the runtime. Existing history is baselined; only a bounded recent tail is eligible when a chat first appears after startup.
